...
def calc_perimeter(merged_xarray: xr.Dataset, mhw_id: int) -> list:
    """
    ...
    timesteps = np.arange(utils.calc_initialization(merged_xarray, mhw_id)[:2], duration)
...
def calc_spatial_cross_correlation(merged_xarray: xr.Dataset, mhw_id: int) -> np.ndarray:
    """
...
def calc_ratio_convexhullarea_vs_objarea(merged_xarray: xr.Dataset, mhw_id: int) -> List[float]:
    """
    ...
    -------
    List[float]
        A list of floats representing the ratio of the area of the object to the area of its convex hull.
        The length of the list is equal to the number of timesteps for which the object is present in the dataset.
    """
    one_obj = merged_xarray.labels.where(merged_xarray.labels==mhw_id)
    ratio_imoverchull_ls = [np.sum(img_as_float(convex_hull_image(one_obj[i].values == mhw_id)))
                            / np.sum(img_as_float(one_obj[i].values == mhw_id))
                            for i in range(len(one_obj.time))]
    
    return ratio_imoverchull_ls

def calc_centroids_per_timestep(forOneMHW_onlylabels_timesteps: xr.DataArray, timestep: int) -> list:
    """
    ...
    """
    sub_labels = forOneMHW_onlylabels_timesteps.isel(time=timestep)
...
def calc_com_per_timestep(forOneMHW_onlylabels_timesteps: xr.DataArray, 
    ...
    sub_labels = xr.DataArray(utils._get_labels(timestep_of_interest), dims=timestep_of_interest.dims, coords=timestep_of_interest.coords).where(timestep_of_interest>0, drop=False, other=np.nan)
    com_list = []
    
    for i in np.unique(sub_labels[~np.isnan(sub_labels)]):
        ...
        if i not in np.unique(sub_labels[:,-1:]) and i not in np.unique(sub_labels[:,:1]):
            ...
            com_list.append(utils._get_center_of_mass(sub_SSTAs_i.where(sub_labels_i > 0, 1.0))[0])
            ...
            if lon_edge < 358.75:
                ...
                com_list.append(utils._get_center_of_mass(append_east_SSTA.where(append_east > 0, 1.0))[0])
...
def calc_displacement(
    ...
    def get_coords(da: xr.DataArray) -> List[Tuple[float, float]]:
        """Returns a list of (lat, lon) tuples for each labeled region in a 2D or 3D DataArray."""
        coords = []
        for i in range(da.shape[0]):
            labels_i = xr.where(da[i, :] > 0, 1, np.nan)
            com = ndimage.center_of_mass(labels_i.data)
            cent = ndimage.measurements.center_of_mass(labels_i.data)
            if np.isnan(com[0]) or np.isnan(cent[0]):
                continue
            lat = convert_lat_range(com[0])
            lon = convert_lon_range(com[1])
            cent_lat = convert_lat_range(cent[0])
            cent_lon = convert_lon_range(cent[1])
            coords.append((lat, lon))
            centroid_xrcoords_ls.append((lat, lon))
            cent_coords.append((cent_lat, cent_lon))
            com_xrcoords_ls.append((cent_lat, cent_lon))
        return coords

    # Set up interpolation functions to convert coordinates to (-90, 90) for latitude and (-180, 180) for longitude.
    lon_range = np.array([da[lon_dim].min(), da[lon_dim].max